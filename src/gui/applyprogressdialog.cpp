...
void ApplyProgressDialog::onCancelButton()
{
    if (operationRunner().isRunning()) {
        ...
        if (KMessageBox::warningTwoActions(this, xi18nc("@info", "Stopping now may cause data loss or drive errors."), xi18nc("@title:window", "Stop Running Operations"), KGuiItem(xi18nc("@action:button", "Stop Anyway"), QStringLiteral("process-stop")), KGuiItem(xi18nc("@action:button", "Keep Running"))) == KMessageBox::PrimaryAction)
